...
import { QrCode, Plus, Download, Package, Store, ChevronDown, ChevronRight, CheckSquare, Square, Search, Filter, X } from 'lucide-react';
...
const QRCodeManagement: React.FC<QRCodeManagementProps> = ({ token, onStatsUpdate }) => {
  ...
  const fetchData = async () => {
    try {
      const [batchesResponse, productsResponse, shopsResponse] = await Promise.all([
        fetch('http://3.7.69.235:3001/api/admin/qrcodes/batches', {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch('http://3.7.69.235:3001/api/admin/products', {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch('http://3.7.69.235:3001/api/admin/shops', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);
  ...
  const fetchBatchQRCodes = async (batchId: string) => {
    try {
      const response = await fetch(`http://3.7.69.235:3001/api/admin/qrcodes/batch/${batchId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://3.7.69.235:3001/api/admin/qrcodes/generate', {
        method: 'POST',
        headers: {
  ...
  const handleAssign = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://3.7.69.235:3001/api/admin/qrcodes/assign', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDownloadPDF = async (batchId: string) => {
    try {
      ...
      const response = await fetch(`http://3.7.69.235:3001/api/admin/qrcodes/download-pdf/${product.productId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDownloadSelectedPDF = async () => {
    ...
    try {
      const response = await fetch('http://3.7.69.235:3001/api/admin/qrcodes/download-selected-pdf', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDownloadStickerSheet = async () => {
    ...
    try {
      const response = await fetch('http://3.7.69.235:3001/api/admin/qrcodes/sticker-sheet', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
