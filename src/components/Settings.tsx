...
const Settings: React.FC<SettingsProps> = ({ token }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingCategory 
      ? `http://3.7.69.235:3001/api/admin/categories/${editingCategory._id}`
      : 'http://3.7.69.235:3001/api/admin/categories';
  ...
  const handleDelete = async (categoryId: string) => {
    if (window.confirm('Are you sure you want to delete this category? This action cannot be undone.')) {
      try {
        const response = await fetch(`http://3.7.69.235:3001/api/admin/categories/${categoryId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
