...
const ProductManagement: React.FC<ProductManagementProps> = ({ token, onStatsUpdate }) => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('http://3.7.69.235:3001/api/admin/categories', {
        headers: {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch('http://3.7.69.235:3001/api/admin/products', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingProduct 
      ? `http://3.7.69.235:3001/api/admin/products/${editingProduct._id}`
      : 'http://3.7.69.235:3001/api/admin/products';
  ...
  const handleDelete = async (productId: string) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        const response = await fetch(`http://3.7.69.235:3001/api/admin/products/${productId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
