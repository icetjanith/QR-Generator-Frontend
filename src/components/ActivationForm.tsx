...
const ActivationForm: React.FC = () => {
  ...
  const fetchQRData = useCallback(async () => {
    try {
      const response = await fetch(`http://3.7.69.235:3001/api/public/qr/${serialNumber}`);
  ...
  const handleActivation = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://3.7.69.235:3001/api/public/qr/${serialNumber}/activate`, {
        method: 'POST',
        headers: {
